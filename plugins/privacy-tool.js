...
async (conn, mek, m, { from, isOwner, reply }) => {
    ...
    try {
        ...
        if (blockedUsers.length === 0) {
            return reply("📋 ʏᴏᴜʀ ʙʟᴏᴄᴋ ʟɪsᴛ ɪs ᴇᴍᴘᴛʏ.");
        }
...
cmd({
    pattern: "setppall",
    desc: "Update Profile Picure Privacy",
    category: "privacy",
    react: "🔐",
    filename: __filename
}, 
...
async (conn, mek, m, { from, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    try {
        if (!isOwner) return reply('🚫 *ʏᴏᴜ ᴍᴜsᴛ ʙᴇ an ᴏᴡɴᴇʀ ᴛᴏ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ*');
...
async (conn, mek, m, { from, l, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply }) => {
    ...
    try {
        ...
        if (!validValues.includes(value)) {
            return reply("❌ ɪɴᴠᴀʟɪᴅ ᴏᴘᴛɪᴏɴ. ᴠᴀʟɪᴅ ᴏᴘᴛɪᴏɴs ᴀʀᴇ: 'all', 'contacts', 'contact_blacklist', 'none'.");
        }

        await conn.updateGroupsAddPrivacy(value);
        reply(`✅ ɢʀᴏᴜᴘ ᴀᴅᴅ ᴘʀɪᴠᴀᴄʏ ᴜᴘᴅᴀᴛᴇᴅ to: ${value}`);
    } catch (e) {
        return reply(`*An error occurred while processing your request.*\n\n_Error:_ ${e.message}`);
    }
});
