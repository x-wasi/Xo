...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply("ʀᴇᴀᴅᴍᴇssᴀɢᴇ ғᴇᴀᴛᴜʀᴇ ɪs ɴᴏᴡ disabled.");
    } else {
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply("*ᴀɴᴛɪ ʙᴀᴅ word ғᴇᴀᴛᴜʀᴇ ɪs ɴᴏᴡ disabled*");
    } else {
...
cmd({
  pattern: "deletelink",
  alias: ["linksdelete"],
  desc: "Enable or ᴅɪsᴀʙʟᴇ ᴅᴇʟᴇᴛᴇ_ʟɪɴᴋs ɪɴ ɢʀᴏᴜᴘs",
  category: "group",
  react: "❌",
  filename: __filename
}, async (conn, mek, m, { isGroup, isAdmins, isBotAdmins, args, reply }) => {
  try {
    if (!isGroup) return reply('This ᴄᴏᴍᴍᴀɴᴅ ᴄᴀɴ ᴏɴʟʏ be ᴜsᴇᴅ ɪɴ ᴀ ɢʀᴏᴜᴘ.');
    if (!isBotAdmins) return reply('ʙᴏᴛ ᴍᴜsᴛ ʙᴇ ᴀɴ ᴀᴅᴍɪɴ ᴛᴏ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ.');
    if (!isAdmins) return reply('You ᴍᴜsᴛ ʙᴇ ᴀɴ ᴀᴅᴍɪɴ ᴛᴏ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ.');

    if (args[0] === "on") {
      config.DELETE_LINKS = "true";
      reply("✅ ᴅᴇʟᴇᴛᴇ_ʟɪɴᴋs is ɴᴏᴡ ᴇɴᴀʙʟᴇᴅ.");
    } else if (args[0] === "off") {
